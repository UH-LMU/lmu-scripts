...
#    cmd = ['convert', '-size', str(dataX)+'x'+str(dataY), 'xc:black', composite_small]
#    print >> file, ' '.join(cmd)
#    cmd = ['convert', '-size', str(dataX)+'x'+str(dataY), 'xc:black', composite_large]
#    print >> file, ' '.join(cmd)
    cmd = ['beditimg',  '-create', str(dataX)+','+str(dataY)+',1', composite_small]
    print >> file, ' '.join(cmd)
    cmd = ['beditimg',  '-create', str(dataX)+','+str(dataY)+',1', composite_large]
    print >> file, ' '.join(cmd)
...
#            print >> file, ' '.join(cmd) 
            cmd = ['beditimg', '-create', str(dataX)+','+str(dataY)+',1', '-box', str(startx)+','+str(starty)+ ',1,'+str(dx+1)+','+str(dy+3) + ',1',  '-fill',  '1',mask_small]
            print >> file, ' '.join(cmd)
            cmd = ['bmask', '-dilate', str(overlap),  mask_small, mask_large]
            print >> file, ' '.join(cmd)
#            cmd = ['composite', composite_small, '-compose',  'plus',  mask_small,   composite_small]
#            print >> file, ' '.join(cmd)
#            cmd = ['composite', composite_large, '-compose',  'plus',  mask_large,   composite_large]
#            print >> file, ' '.join(cmd)
            cmd = ['badd', '-output', composite_small, composite_small,  mask_small]
            print >> file, ' '.join(cmd)
            cmd = ['badd', '-output', composite_large, composite_large,  mask_large]
            print >> file, ' '.join(cmd)
    ...
    cmd = ['convert', '-threshold',  '0.5',  composite_small,   composite_small + '.bmp']
    cmd = ['convert',  composite_small,   composite_small + '.bmp']
    print >> file, ' '.join(cmd)
    cmd = ['convert', '-threshold',  '0.5',  composite_large,   composite_large + '.bmp']
    cmd = ['convert',  composite_large,   composite_large + '.bmp']
    print >> file, ' '.join(cmd)
